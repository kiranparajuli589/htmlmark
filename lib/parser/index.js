  ...
  for (let i=0; i<tokens.length; i++) {
    ...
    if (token.type === TOKENS.LIST) {
      ...
      token.items.forEach(listItem => {
        ...
        if (token.checkList) {
          listItemHtml = listItemHtml.replace("%s", `<input type="checkbox"${(listItem.checked) ? " checked" : ""}>${deepTokenize(listItem)}`)
        } else {
          listItemHtml = listItemHtml.replace("%s", deepTokenize(listItem))
        }
