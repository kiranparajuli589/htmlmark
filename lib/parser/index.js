  ...
  #parseList() {
    ...
    this.#currentLexer.items.forEach(listItem => {
      ...
      if (this.#currentLexer.meta.isCheckList) {
        ...
        listItemHtml = listItemHtml.replace("%s", `<input type="checkbox"${isChecked}>${Parser.#parseContent(listItem.tokens)}`)
      } else {
        listItemHtml = listItemHtml.replace("%s", Parser.#parseContent(listItem.tokens))
      }
