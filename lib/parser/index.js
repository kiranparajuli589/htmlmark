  ...
  for (let i=0; i<tokens.length; i++) {
    ...
    if (token.type === TOKENS.LIST) {
      ...
      token.items.forEach(listItem => {
        ...
        if (token.checkList) {
          listItemHtml = listItemHtml.replace("%s", `<input type="checkbox"${(listItem.checked) ? " checked" : ""}>${parseContent(listItem)}`)
        } else {
          listItemHtml = listItemHtml.replace("%s", parseContent(listItem))
        }
