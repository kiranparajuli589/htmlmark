export const REGEX = {
	...
	HEADING: {
		...
		WITH_OR_WITHOUT_FENCE: /^\s*(?<level>#{1,6})\s+((?<v>.+)(?=\s+#+\s*$)|(?<vv>.+))$/g,
		UNDERLINE_1: /^\s*={2,}\s*$/g,
		UNDERLINE_2: /^\s*-{2,}\s*$/g
	},
	...
	PARAGRAPH: {
		...
		REF_LINK: /^((?<!!)\[.+?](?!\(.+\))){1,2}/,
		WITHOUT_BI: /`(?<code>.+?)`(?!`)|~~(?<strike>.+?)~~(?!~)|__(?<underline>.+?)__(?!_)|(?<link>\[.+?]\(\S+(?:\s['"].+['"])?\))|(?<!!)\[(?<ref>.+)](?!\(.+\))|(?<text>.)/g,
		ITALIC_WITH_TEXT: /\*(?<italic>.+?)\*(?!\*)|(?<text>.)/g,
