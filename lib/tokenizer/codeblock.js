...
class CodeBlock {
	...
	constructor(lines, cursor, indentObj, indent, rawIndent) {
		...
		if (this.#withBraces) {
			this.#tickCount = lines[cursor].trim().match(/^`{3,}/g)[0].length
		}
	...
	#findEnd() {
		...
		do {
			...
			if (nextLine !== undefined) {
				if (this.#withBraces) {
					const braceEndRegex = new RegExp("^`{" + this.#tickCount + "}\\s*$", "g")
					nextLineMatch = Utils.testRegex(nextLine.trim(), braceEndRegex)
					if (
						nextLineMatch &&
