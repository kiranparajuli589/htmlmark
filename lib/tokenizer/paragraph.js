...
export class Paragraph {
	...
	/**
	 * Determines if the provided string is evenly fenced by the provided identifier
	 *
	 * @param {String} str
	 * @param {Number} cursor
	...
	static #isEvenFenced(str, cursor, identifier) {
		let start = cursor + identifier.length
   ...
   * 					"title" 50 50 are optional
	...
	static #escapeTextTokens(tokens) {
		if (Array.isArray(tokens)) {
			tokens.forEach(token => {
				if (token.type === TOKENS.TEXT) {
					token.value = Esc.unEscape(token.raw)
					token.value = Esc.nonTags(token.value)
				} else if (token.type === TOKENS.LINK) {
					Paragraph.#escapeTextTokens(token.tokens.title.tokens)
				} else if (token.type === TOKENS.IMAGE) {
					Paragraph.#escapeTextTokens(token.tokens.alt.tokens)
				} else {
					Paragraph.#escapeTextTokens(token.tokens)
				}
			})
		}
		return tokens
	...
	static tokenize(lineToParse, linkRefs) {
		const pTokens = Paragraph.#findEmphasis(lineToParse, linkRefs)

		// escape the text token raw into value
		const eTokens = Paragraph.#escapeTextTokens(pTokens)
		return eTokens
	}
}
