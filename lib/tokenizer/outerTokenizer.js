  ...
  for(let i=0; i<lines.length; i++) {
    ...
    if (beforeCompare === tokenizedContent.length) {
      tokenizedContent.push({
        type: "paragraph",
        tokens: {
          value: lineToParse // only \r are removed
        },
        // https://github.com/kiranparajuli589/markdown-parser/runs/7184657512?check_suite_focus=true
        raw: process.env.CI === "true" ? lineToParse : lines[i],
      })
    }
