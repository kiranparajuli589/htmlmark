...
export class Heading {
	...
	static testRegex(text) {
		return Utils.testRegex(text, REGEX.HEADING.WITH_OR_WITHOUT_FENCE)
	}
	 ...
	 */
	static match(text) {
		return Utils.execRegex(text, REGEX.HEADING.WITH_OR_WITHOUT_FENCE)
	}
	...
	tokenize() {
		if (!this.#uType) {
			return {
				...
				value: Esc.nonTags(this.#match.value.trimEnd()),
				raw: this.#line,
				uType: false
			}
		} else {
			return {
				level: this.#level,
				value: Esc.nonTags(this.#line.trimEnd()),
				raw: `${this.#line}\n${this.#nextLine}`,
				uType: true
			}
		}
