...
class Table {
	 ...
	 * @param {number} count
	 * @param {number} indent
	 * @param {Object} indentObj
	 *
	 * @returns {boolean}
	 */
	static #isHBSep(text, count, indent, indentObj) {
		return indentObj.get(text) === indent &&
			(
				Utils.testRegex(text, REGEX.TABLE.DASH_LINE) ||
				Utils.testRegex(text, REGEX.TABLE.COLON_LINE) ||
				Utils.testRegex(text, REGEX.TABLE.S_DASH_LINE) ||
	 ...
	 * @returns {boolean}
	 */
	static #isRow(text, count, indent, indentObj) {
		return indentObj.get(text) === indent &&
			Table.#testRow(text) &&
			Table.#getCellCount(text) === count &&
			!Table.#isHBSep(text, count, indent, indentObj)
	}
	 ...
	 * @returns {boolean}
	 */
	static test({ lines, cursor, indent, indentObj }) {
		...
		if (nextLine !== undefined) {
			...
			if (
				(
					nextNextLine
					&& Table.#isHBSep(nextLine, cellCount, indent, indentObj)
					&& Table.#isRow(nextNextLine, cellCount, indent, indentObj)
				)
				|| Table.#isRow(nextLine, cellCount, indent, indentObj)
			) {
				return true
			}
		}
	 ...
	 * @returns {Table}
	 */
	constructor(lines, cursor, indentObj, indent, linkRefs) {
		this.#lines = lines
		this.#cursor = cursor
		this.#start = cursor
		this.#indent = indent
	...
	#findEnd() {
		...
		this.#cursor += 1
		if (Table.#isHBSep(this.#lines[this.#cursor], this.#cellCount, this.#indent, this.#indentObj)) {
			this.#cursor += 2
		...
		while (
			...
			Table.#isRow(this.#lines[this.#cursor], this.#cellCount, this.#indent, this.#indentObj)
		) {
			this.#cursor++
		}
	...
	static parse(lexer) {
		...
		const tBodyHtml = tBody.map(row => `
    <tr><td>${row.map(cell => Paragraph.parse(cell)).join("</td><td>")}</td></tr>`).join("")
