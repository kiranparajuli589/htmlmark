...
export class Quote {
	...
	static tokenize(lines, cursor, indent) {
		...
		const nestedBody = this.nestBody(shrunkBody)
	...
	static nestBody(shrunkBody) {
		const result = []

		let lastResult, lastPush

		shrunkBody.forEach(item => {
			// at this point we've already calculated the common depth
			// so quote separator is useless now
			...
			if (
				...
				lastPush.raw = lastPush.raw.concat(item.raw)
