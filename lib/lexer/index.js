...
class Lexer {
	...
	#skipFrontMatter() {
		const frontMatterEnd = FrontMatter.test(this.#lines)
		if (frontMatterEnd !== -1) {
			this.#lines = this.#lines.slice(frontMatterEnd)
			this.#cursor = 0
		}
	}
	...
	getFrontMatter() {
		const frontMatterEnd = FrontMatter.test(this.#lines)
		if (frontMatterEnd !== -1) {
			this.#frontMatter = new FrontMatter(this.#lines.slice(0, frontMatterEnd))
