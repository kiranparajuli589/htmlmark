...
import { FrontMatter } from "../tokenizer/frontMatter.js"
...
export class Lexer {
	...
	constructor(lines, from = null, frontMatter = false) {
		this.#lines = lines
		this.#lexerData = []
		this.#from = from
		this.#cursor = 0
	...
	#runCurrLineLexer() {
		...
		if (HTML.testBlock(this.#lines, this.#cursor, this.#currLineIndent)) return this.#runHTMLLexer()
	...
	#checkForFrontMatter() {
		const frontMatter = new FrontMatter(this.#lines)
		const isPresent = frontMatter.checkIfPresent()
		if (isPresent) {
			this.#lexerData.push({
				type: TOKENS.FRONT_MATTER,
				value: frontMatter.getValue()
			})
			this.#lines = frontMatter.getUpdatedLines()
		}
	}

	getFrontMatter() {
		this.#checkForFrontMatter()
		if (this.#lexerData[0] && this.#lexerData[0].type === TOKENS.FRONT_MATTER) {
			return this.#lexerData[0].value
		} else {
			return null
		}
	}
