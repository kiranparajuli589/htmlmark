const { getIndent, tokenizeParagraph, tokenizeTableCell, isListItemOfOrderedType, getListItemTokens } = require("../regex/helper")
...
const lexer = (lines) => {
  ...
  for(let i=0; i<lines.length; i++) {
    ...
    if (lineToParse.match(REGEX.LIST_ITEM)) {
      ...
      const typeOfList = isListItemOfOrderedType(lineToParse)
      ...
      while (
        ...
        isListItemOfOrderedType(lines[cursorPosition]) === typeOfList
      ) {
      ...
      Object.keys(listBody).forEach(listItem => {
        ...
        listItemLexer.tokens = tokenizeParagraph(listBody[listItem].value)
    ...
    if (lineToParse.match(REGEX.QUOTE)) {
      ...
      lexerContent.push({
        ...
        indent: currentLineIndentation,
        depth,
        tokens: tokenizeParagraph(lineValue),
        raw: lineToParse
      })
    }
    ...
    for (let key in REGEX.COMMON) {
      ...
      if (match) {
        ...
        if (TOKENS[key] === TOKENS.CHECKBOX) {
          // preserve the checked status
          lexerData.checked = match.groups.checked === "x"
          delete match.groups.checked
        }
    ...
    if (lengthBeforeCompare === lexerContent.length) {
      lexerContent.push({
        type: TOKENS.PARAGRAPH,
        indent: currentLineIndentation,
        tokens: tokenizeParagraph(lineToParse.trim()),
