const { getIndent, find, matchTableCell } = require("../regex/helper")
...
const lexer = (lines) => {
  ...
  for(let i=0; i<lines.length; i++) {
    ...
    if (lineToParse.match(REGEX.TABLE_ROW)) {
      ...
      const tIndent = getIndent(lineToParse)
      const nextLineIndent = getIndent(nextLine)

      if (!nextLine || !nextNextLine || tIndent !== nextLineIndent) {
        lexerContent.push({
          type: TOKENS.PARAGRAPH,
          tokens: find(lineToParse),
          indent: tIndent
        })
        continue
      }
      ...
      if (isTableHeadingBodySeparatorPresent) {
        ...
        const isTableRow = nextNextLine.match(REGEX.TABLE_ROW)
        if (isTableRow && getIndent(nextNextLine) === tIndent) {
          ...
          while(
            lines[tBodyLine] &&
            getIndent(lines[tBodyLine]) === tIndent &&
            lines[tBodyLine].match(REGEX.TABLE_ROW)
          ) {
            tBodyLine++
          }
          ...
          if (tBodyLine > i + 3) {
            const tableRows = lines.slice(i+2, tBodyLine)
            const tableLexer = {
              type: TOKENS.TABLE,
              rows: []
            }
            // now tokenize the table heading first
            tableLexer.rows.push(matchTableCell(lineToParse))
            // no actions required for the separator line
            // move on to parse the table body items
            for(let j=0; j<tableRows.length; j++) {
              tableLexer.rows.push(matchTableCell(tableRows[j]))
            }
    ...
    for (let key in REGEX.COMMON) {
      ...
      if (match) {
        ...
        if (TOKENS[key] === TOKENS.HEADING) {
          ...
          lexerData.tokens = find(match.groups.value)
        ...
        if (TOKENS[key] === TOKENS.QUOTE) {
          ...
          lexerData.tokens = find(quoteLine)
